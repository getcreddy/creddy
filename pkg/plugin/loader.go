...
// Reload re-scans the plugins directory and loads any new plugins.
// Returns a list of plugin names that were loaded during this reload.
func (l *Loader) Reload() ([]string, error) {
	...
	for _, name := range plugins {
		l.mu.RLock()
		_, alreadyLoaded := l.plugins[name]
		l.mu.RUnlock()
		...
		if _, err := l.LoadPlugin(name); err != nil {
			l.logger.Warn("failed to load plugin during reload", "name", name, "error", err)
		} else {
